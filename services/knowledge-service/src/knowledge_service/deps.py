...
The knowledge graph is global (no RLS), so DB access is via ``admin_session``;
the event bus + Redis are held for uniform readiness + the served-graph cache
...
from knowledge_service.settings import ServiceSettings
