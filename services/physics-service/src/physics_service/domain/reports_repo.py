"""physics_reports repository (M11 Step 7).

Tenant-scoped (RLS) store, idempotent on ``(tenant_id, correlation_id)`` so a
re-delivered stroke updates one report rather than duplicating (NFR-M11-03).
"""
...
_COLUMNS = (
    ...
    "quantities, kinetic_chain, quality, schema_version, model_version, "
        ...
        (
            await session.execute(
                text(
                    ...
                    "INSERT INTO physics_reports "  # nosec B608
                    "  (id, tenant_id, correlation_id, person_id, shot_type, shot_confidence, "
                    "   quantities, kinetic_chain, quality, schema_version, model_version, "
                    "   out_of_expected_range, provisional) "
                    "VALUES (:id, :tid, :corr, :pid, :st, :sc, cast(:q as jsonb), "
                    "        cast(:kc as jsonb), cast(:ql as jsonb), :sv, :mv, :oor, :prov) "
                    ...
                    "  quantities = EXCLUDED.quantities, kinetic_chain = EXCLUDED.kinetic_chain, "
                ...
                ),
                {
                    ...
                    "st": shot_type,
                    "sc": shot_confidence,
                    "q": json.dumps(quantities),
                    "kc": json.dumps(kinetic_chain),
                    "ql": json.dumps(quality),
                    "sv": schema_version,
                    "mv": model_version,
...
async def get_report(session: AsyncSession, correlation_id: str) -> dict[str, Any] | None:
    ...
    query = f"SELECT {_COLUMNS} FROM physics_reports WHERE correlation_id = :corr"  # nosec B608
