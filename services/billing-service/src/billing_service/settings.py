...
class ServiceSettings(CoreSettings):
    ...
    service_name: str = Field("billing-service")
    ...
    redis_url: str = Field(
        ...
        description="Redis URL for the idempotency store + entitlement cache",
    )

    # Payment provider webhook signing secret (Step 6). The fake provider in
    # dev/test HMAC-signs simulated webhooks with this; a real provider
    # supplies its own signing secret via the managed secret store.
    payment_webhook_secret: str = Field(
        "dev-webhook-secret-not-for-production",
        description="HMAC secret used to verify payment-provider webhooks",
    )
