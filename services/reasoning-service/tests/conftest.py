"""Shared fixtures for reasoning-service tests."""
...
# M13 read access is consent-scoped, so the identity schema must exist for the
# integration tests.
...
def _migrated_database() -> str:
    """Apply base + identity + reasoning migrations once (idempotent)."""
    url = _database_url()
    ...
    try:
        async with admin_session(session_factory) as session:
            await session.execute(
                ...
                {"id": tid, "name": f"rsn-svc-{uuid.uuid4().hex[:8]}"},
            )
        yield tid
    finally:
