...
from bat_service import __version__
from bat_service.deps import build_deps, shutdown_deps
from bat_service.health import router as health_router
from bat_service.health import version_router
from bat_service.routes import bat_router, internal_router
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="CIP bat-service",
        version=__version__,
        description=(
            "M07 Bat Detection. Consumes video.normalized (with M06 wrists for "
            "hand-bat association), locates the bat per frame, derives bat "
            "angle and swing plane, and publishes bat.tracked."
        ),
        lifespan=lifespan,
    )
