"""Shared fixtures for academy-service tests."""
...
# M18 access control is consent/membership-scoped, so the identity schema
# must exist for the integration tests.
...
def _migrated_database() -> str:
    """Apply base + identity + academy migrations once (idempotent)."""
    url = _database_url()
    ...
    try:
        async with admin_session(session_factory) as session:
            await session.execute(
                ...
                {"id": tid, "name": f"acad-svc-{uuid.uuid4().hex[:8]}"},
            )
        yield tid
    finally:
        await engine.dispose()
