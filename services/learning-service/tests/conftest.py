"""Shared fixtures for learning-service tests."""
...
from learning_service.main import create_app
...
# M17 read access is consent-scoped, so the identity schema must exist for the
# integration tests.
...
def _migrated_database() -> str:
    """Apply base + identity + learning migrations once (idempotent)."""
    url = _database_url()
    ...
    try:
        async with admin_session(session_factory) as session:
            await session.execute(
                ...
                {"id": tid, "name": f"lrn-svc-{uuid.uuid4().hex[:8]}"},
            )
        yield tid
    finally:
