"""Shared fixtures for shot-service tests.
...
# M09's annotation consent gate reads M02-owned tables (persons, consents, guardianships),
    ...
    try:
        async with admin_session(session_factory) as session:
            await session.execute(
                ...
                {"id": tid, "name": f"shot-svc-{uuid.uuid4().hex[:8]}"},
            )
        yield tid
    finally:
