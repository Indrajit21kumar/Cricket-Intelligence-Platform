...
from ball_service import __version__
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="CIP ball-service",
        version=__version__,
        description=(
            "M08 Ball Tracking. Consumes video.normalized, detects the ball "
            "under blur, derives release/bounce/contact plus line, length and "
            "an ESTIMATED speed, and publishes ball.events."
        ),
        lifespan=lifespan,
    )
