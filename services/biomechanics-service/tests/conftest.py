"""Shared fixtures for biomechanics-service tests.
...
# M10 reads M02-owned tables (persons, consents, guardianships) through the
    ...
    try:
        async with admin_session(session_factory) as session:
            await session.execute(
                ...
                {"id": tid, "name": f"bio-svc-{uuid.uuid4().hex[:8]}"},
            )
        yield tid
    finally:
